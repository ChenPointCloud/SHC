...
class TestData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        pcd1 = self.source[index][:self.num_points]
        pcd2 = self.target[index][:self.num_points]
...
class TrainData(Dataset):
    ...
    def __getitem__(self, index):
        pcd1 = self.points[index][:self.num_points]
        pcd2 = self.points[index][:self.num_points]
        transform = random_pose(self.max_angle, self.max_trans)
        pose1 = random_pose(np.pi, self.max_trans)
        pose2 = transform @ pose1
        pcd1 = pcd1 @ pose1[:3, :3].T + pose1[:3, 3]

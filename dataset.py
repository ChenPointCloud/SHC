#the code is referred from: https://github.com/supersyq/RIENet
...
def get_datasets(args):
    if args.dataset == '7scenes':
        if not args.eval:
            ...
            dataset = Scene7('data/7scene', transform=transform, classinfo=cinfo)
            trainset = TransformedDataset(dataset, partition='train',n_subsampled_points=args.sub_points)
            ...
            testdata = Scene7('data/7scene', transform=transform, classinfo=cinfo)
            testset = TransformedDataset(testdata, partition = 'test',n_subsampled_points=args.sub_points)
